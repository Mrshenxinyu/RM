...
uint16_t random_i = 200;
//	double rand_i=-2;
//	float rand_f=0.1;
//	float rand_s=0.1;
//	unsigned int seed = 42;  // 初始种子
...
//	double random() {
//    seed = (1664525 * seed + 1013904223) % 4294967296;  // 线性同余生成器
//    return ((double)seed / 4294967296.0) * 4.0 - 2.0;  // 缩放到-2到2之间
//	}
/***********************************************************
*@fuction	:random
*@brief		:
...
double random()
{
    //		rand_i = rand_i + rand_f;
    //		rand_f = rand_f + rand_s;
    //		if(rand_i > 2 || rand_i<-2)
    //		{
    //			rand_f=-rand_f;
    //			rand_s = -rand_s;
    //		}
    ...
    if (increasing)
    {
        seed = (seed * 1103515245 + 12345) & 0x7FFFFFFF; // 简单的伪随机数生成器
        increment = ((double)seed / 0x7FFFFFFF) * 0.2; // 随机递增速度在0到0.2之间
        number += increment;
        if (number >= 2.0)
        {
            increasing = 0; // 达到2后切换到递减
        }
    }
    else
    {
        seed = (seed * 1103515245 + 12345) & 0x7FFFFFFF; // 简单的伪随机数生成器
        increment = ((double)seed / 0x7FFFFFFF) * 0.2; // 随机递减速度在0到0.2之间
        number -= increment;
        if (number <= -2.0)
        {
            increasing = 1; // 达到-2后切换到递增
        }
    }
    return number;
}
...
static void  chassis_gyroscope(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
...
void chassis_behaviour_mode_set(chassis_move_t *chassis_move_mode)
{
    ...
    if (chassis_move_mode->chassis_RC->key.v & CHASSIS_SPIN_KEYBOARD)
    {
        //		 flag = 1;
        //		if(flag == 1)
        //		{
        ...
        //			k = 1.5;
        //		}
        //		if(flag == 1 && chassis_move_mode->chassis_RC->key.v & CHASSIS_SPIN_KEYBOARD)
        //		{
        chassis_behaviour_mode = CHASSIS_SPIN;
        k = 1.5;
        //		}
...
static void  chassis_gyroscope(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    ...
    chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
    *wz_set = (4 - fabs(chassis_move_rc_to_vector->vx_set) * 0.5 - fabs(chassis_move_rc_to_vector->vy_set) * 0.5) * k; //正 1.3
    //	*wz_set = k;  //正 1.3
    //小陀螺速度解算
}
