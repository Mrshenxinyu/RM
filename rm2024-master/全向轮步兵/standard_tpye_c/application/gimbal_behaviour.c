...
void gimbal_behaviour_mode_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if (gimbal_behaviour == GIMBAL_CV)  // 2023.3.13添加 CV控制模式
		{
...
void gimbal_behaviour_control_set(fp32 *add_yaw, fp32 *add_pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_MOTIONLESS)
    {
		...
		else if (gimbal_behaviour == GIMBAL_CV)  // 2023.3.13新增 CV控制模式
		{
...
////				else
////				{
//					/* 2023.3.19添加 CV模式 (只有在switch置下时才有效)*/
...
//		y_filtered = gimbal_control_set->can_CV->y;
		...
		//PID
//		int16_t xe=0, ye=0;
//		xe = x_filtered - CV_SIZE_X/2;
//		ye = y_filtered - CV_SIZE_Y/2;
//		if(xe > CV_DEADBAND || xe < -CV_DEADBAND)
//		{
//			*yaw = -PID_calc(&gimbal_control_set->CV_x_pid, x_filtered, CV_SIZE_X/2);
//		}
//		if(ye > CV_DEADBAND || ye < -CV_DEADBAND)
//		{
//			*pitch = PID_calc(&gimbal_control_set->CV_y_pid, y_filtered, CV_SIZE_Y/2);
//		}
		
		*yaw = gimbal_control_set->can_CV->x * 0.01745f;  // ??YAW_RC_SEN
		*pitch = gimbal_control_set->can_CV->y * 0.01745f;
}
