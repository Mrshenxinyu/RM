...
void shoot_init(void)
{
    ...
    ramp_init(&shoot_control.fric1_ramp, SHOOT_CONTROL_TIME * 0.001f, FRIC_TEST, FRIC_OFF);
    ramp_init(&shoot_control.fric2_ramp, SHOOT_CONTROL_TIME * 0.001f, FRIC_TEST, FRIC_OFF);
...
//        shoot_control.trigger_motor_pid_angle.max_iout = TRIGGER_BULLET_PID_MAX_IOUT;
    ...
    else
    {
        ...
        shoot_control.given_current = (int16_t)(shoot_control.trigger_motor_pid_speed.out);
...
static void shoot_bullet_control(void)
{
    ...
    {			 
			...
			else if((int16_t)(shoot_control.trigger_motor_pid_angle.out) < -TRIGGER_SPEED)
			{
				shoot_control.trigger_speed_set = -TRIGGER_SPEED;
			}
			else
        ...
        //没到达一直设置旋转速度
//			  shoot_control.trigger_speed_set = -(int16_t)(shoot_control.trigger_motor_pid.out);
//				shoot_control.trigger_speed_set = TRIGGER_SPEED;
